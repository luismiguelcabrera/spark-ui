...
function TestForm({ onSubmit = vi.fn() }: { onSubmit?: (data: { name: string }) => void }) {
  ...
  return (
    <RHFForm form={form} onSubmit={onSubmit} aria-label="test form">
...
describe("RHFForm", () => {
  ...
  it("has displayName", () => {
    expect(RHFForm.displayName).toBe("RHFForm");
  });
  ...
  it("merges className", () => {
    function FormWithClass() {
      const form = useForm({ defaultValues: { name: "" } });
      return (
        <RHFForm form={form} onSubmit={vi.fn()} className="custom" aria-label="test">
